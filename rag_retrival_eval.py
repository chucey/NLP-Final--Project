...
This file is slightly modified from the original `rag_retieval.py` and is used for evlauating the retrieval.
"""
...
def load_vectorstore(index_dir: str ="faiss_yelp",
    ...
    Args:
        ...
        model_name (str, optional): name of the embedding model to use. Defaults to "sentence-transformers/all-MiniLM-L6-v2".
...
def retrieve_reviews_for_summary(vs: FAISS,
    ...
    Args:
        ...
        query (str, optional): The query to use for similarity search. Defaults to "overall customer experience food quality service atmosphere value wait time cleanliness".
    ...
    if metadata_filter:
        ...
        if not matches:
            ...
            query = query if query else "overall customer experience food quality service atmosphere value wait time cleanliness"
    ...
    formatted_reviews = [
        doc.metadata.get("review_id", "N/A") for doc in matches[:k]
           ]
    return set(formatted_reviews)

# vs = load_vectorstore()
# results = retrieve_reviews_for_summary(vs, categories="Italian", k=25)
